...
module.exports = {
	...
	baseApi(sub_url, method, json_data, cb) {
		...
		let request = {
			...
			headers : {
				...
				Authorization  : Cache.currentUser ? 'bearer ' + Cache.token : null
			}
		};
	...
	confirmOTP(PhoneNumber, OtpNumber, PushToken, cb) {
		this.baseApi('/api/otpLogin', 'POST', { PhoneNumber, OtpNumber, PushToken }, (err, res) => {
	...
	async uploadImage(uri, cb) {
		try {
			...
			let response = await fetch(config.SERVICE_API_URL + 'UpLoad/UploadProfileImage', {
				method  : 'POST',
				headers : {
					Accept         : 'application/json',
	...
	login(Username, Password, cb) {
		...
		this.baseApi('Account/Login', 'POST', { Username, Password }, cb);
	},
	...
	checkAndAddUser(email_id, first_name, last_name, cb) {
		this.baseApi('checkAndAddUser', 'POST', { email_id, first_name, last_name }, cb);
	},

	updateHomeLocation(email_id, lat, long, cb) {
		this.baseApi('updateHomeLocation', 'POST', { email_id, lat, long }, cb);
	},

	updatePhoneNumber(phone_no, email_id, login_mode, cb) {
		this.baseApi('updatePhoneNumber', 'POST', { phone_no, email_id, login_mode }, cb);
	}
};
